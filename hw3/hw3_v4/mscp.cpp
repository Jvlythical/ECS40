...
int ply;         /* Number of half-moves made in game and search */
//int WTM() (~ply & 1); /* White-to-move predicate */
unsigned long zobrist[12][64];   /* Hash-key construction */
unsigned long hash_stack[1024]; /* History of hashes, for repetition */
signed char undo_stack[6*1024], *undo_sp; /* Move undo administration */
struct move move_stack[1024], *move_sp; /* History of moves */
...
const unsigned int PRESCORE_EQUAL   =    (10U<<9);
const unsigned int PRESCORE_HASHMOVE  =   (3U<<14);
const unsigned int PRESCORE_KILLERMOVE = (2U<<14);
const unsigned int PRESCORE_COUNTERMOVE = (1U<<14);
...
transposition_table_entry *TTABLE = (core.tt);
opening_book_entry *BOOK = (core.bk);
...
inline char CHAR2RANK(int c)          { return ((c)-'1'); }   /* text to rank */
...
{
        board.print_square(FR(move));
        board.print_square(TO(move));
        if (move >= SPECIAL) {
        ...
        *num = n;
        ...
        while (move_sp > m) {
                ...
                if (move) {
                        ...
                        if (piece) {
                                puts("ambiguous!"); board.print_board(); puts(line);
                                move_sp = m;
                                return 0;
                        }
...
static void load_book(const char *filename)
{
        ...
        while (readline(line, sizeof(line), fp) >= 0) {
                ...
                for (;;) {
                        ...
                        s += num;
                        if (booksize < CORE) {
                                BOOK[booksize].hash = board.compute_hash();
...
static int qsearch(int alpha, int beta)
{
        ...
        hash_stack[ply] = board.compute_hash();
        best_score = evaluate();
        if (best_score >= beta) {
                return best_score;
        }
...
//		std::cout << setw(4) << setprecision(2) << (best_score/100.0);	
		//cout << setw(5) << nodes << setw(3) << depth << setw(10) << setprecision(2) << best_score/100.0 << setprecision(ss);
		
								
                //printf(" %5lu %3d %+1.2f ", nodes, depth, best_score / 100.0);
                cout << setw (6) << nodes << setw (4) << depth << " ";
                cout << setprecision (2) << "+" << (best_score / 100.0) << " ";
                print_move_san(move);
                puts("");
...
static void cmd_list_moves(const char *dummy)
{
	...
	string temp = (nmoves == 1 ? "" : "s");
        cout << nmoves << "move" << temp << endl;
        //printf("%d move%s\n", nmoves, nmoves==1 ? "" : "s");
}
...
{
        board.setup_board("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq -");
        load_book("book.txt");
        computer[0] = 0;
        computer[1] = 1;
        rnd_seed = time(NULL);
...
static void cmd_help(const char *dummy)
{
        ...
        c = mscp_commands;
        do {
                cout << setw(8) << (c->name ? c->name : "") << " - " << c->help << endl;
